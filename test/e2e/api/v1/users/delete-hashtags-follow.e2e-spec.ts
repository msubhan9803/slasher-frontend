...
describe('Delete Hashtags Follow (e2e)', () => {
  ...
  describe('Delete /api/v1/users/:userId/followed-hashtags/:hashtag', () => {
    it('requires authentication', async () => {
      const hashtag1 = String;
      const userId = new mongoose.Types.ObjectId();
      await request(app.getHttpServer()).delete(
        `/api/v1/users/${userId}/followed-hashtags/${hashtag1}`,
      ).expect(HttpStatus.UNAUTHORIZED);
    });

    it('delete the hashtag follows successful if parameter hashtag and userId is exists', async () => {
      const response = await request(app.getHttpServer())
        .delete(`/api/v1/users/${activeUser._id.toString()}/followed-hashtags/${hashtag[0].name}`)
    ...
    it('returns the expected response when the hashtag is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .delete(`/api/v1/users/${activeUser._id.toString()}/followed-hashtags/${hashtagName}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it("returns the expected error response when a user tries to get another user's hashtag-follow status", async () => {
      ...
      const response = await request(app.getHttpServer())
        .delete(`/api/v1/users/${differentUserId}/followed-hashtags/${hashtag[0].name}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    describe('Validation', () => {
      it('userId must be a mongodb id', async () => {
        ...
        const response = await request(app.getHttpServer())
          .delete(`/api/v1/users/${activeUserId}/followed-hashtags/${hashtag[0].name}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
