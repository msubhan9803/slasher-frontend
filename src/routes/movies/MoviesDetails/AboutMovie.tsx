...
function AboutMovie() {
  ...
  const changeTab = (tab: string) => {
    if (!queryParam || queryParam !== 'self') {
      navigate(`/movies/1/${tab}`);
    } else {
      navigate(`/movies/1/${tab}?view=self`);
    }
  };
  useEffect(() => {
    if (params.id === 'edit' && queryParam !== 'self') { navigate('/movies/1/details'); }
  });
  ...
  return (
    <div>
      <div className="bg-dark my-3 p-4 pb-0 rounded-2">
        ...
        <Row className="justify-content-center justify-content-xl-start">
          <Col xs={queryParam === 'self' ? 12 : 5} md={4} lg={queryParam === 'self' ? 7 : 6} xl={5}>
            <TabLinks tabLink={tabs} setSelectedTab={changeTab} selectedTab={params.id} className="justify-content-around justify-content-xl-start" />
          </Col>
        </Row>
      </div>
      {
      ...
      {queryParam === 'self' && params.id === 'edit' && <MovieEdit />}
    </div>
  );
}
