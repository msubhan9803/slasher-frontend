...
describe('Followed Hashtags (e2e)', () => {
  ...
  describe('PUT /api/v1/users/:userId/hashtag-follows/:hashtag', () => {
    ...
    it('get the hashtag follows successful if parameter hashtag and userId is exists and notification is true', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/v1/users/${activeUser._id.toString()}/hashtag-follows/${hashtag[0].name}`)
    ...
    it('get the hashtag follows successful if parameter hashtag and userId is exists and notification is false', async () => {
      const response = await request(app.getHttpServer())
        .put(`/api/v1/users/${activeUser._id.toString()}/hashtag-follows/${hashtag[0].name}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
    ...
    it('returns the expected response when the hashtag is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .put(`/api/v1/users/${activeUser._id.toString()}/hashtag-follows/${hashtagName}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send({ notification: true });
    ...
    it("returns the expected error response when a user tries to get another user's hashtag-follow status", async () => {
      ...
      const response = await request(app.getHttpServer())
        .put(`/api/v1/users/${differentUserId}/hashtag-follows/${hashtag[0].name}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send({ notification: true });
    ...
    describe('Validation', () => {
      it('userId must be a mongodb id', async () => {
        ...
        const response = await request(app.getHttpServer())
          .put(`/api/v1/users/${activeUserId}/hashtag-follows/${hashtag[0].name}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send({ notification: true });
