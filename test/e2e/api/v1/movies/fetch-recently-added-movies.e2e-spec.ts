...
describe('Recently Added Movies (e2e)', () => {
  ...
  describe('GET /api/v1/movies/recently/added?limit=', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get('/api/v1/movies/recently/added').expect(HttpStatus.UNAUTHORIZED);
    });
    ...
    it('transforms the logo field into a full Movie DB URL', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get('/api/v1/movies/recently/added?limit=1&sortBy=name')
    ...
    it('when logo is null than expected response', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get('/api/v1/movies/recently/added?limit=1')
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it('when sortBy is name than expected all movies response', async () => {
      ...
      const limit = 10;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'name'}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it('when sortBy is releaseDate than expected all movies response', async () => {
      ...
      const limit = 10;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'releaseDate'}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it('when sortBy is rating than expected all movies response', async () => {
      ...
      const limit = 10;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'rating'}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
      for (let i = 1; i < response.body.length; i += 1) {
    ...
    it('when name contains supplied than expected all movies response', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'rating'}&nameContains=${nameContains}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it('when the startsWith is exist than expected response', async () => {
      ...
      const limit = 3;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'name'}&startsWith=${sortNameStartsWith}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
      expect(response.body).toEqual([{
        _id: expect.stringMatching(SIMPLE_MONGODB_ID_REGEX),
    ...
    it('when startsWith is not exist and nameContains is exist than expected response', async () => {
      ...
      const limit = 3;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'name'}&nameContains=${nameContains}&startsWith=${sortNameStartsWith}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
    ...
    it('returns the data which are added in last 30 days', async () => {
      ...
      const limit = 10;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/movies/recently/added?limit=${limit}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
      expect(response.body).toMatchObject([
        {
    ...
    describe('when `after` argument is supplied', () => {
      ...
      it('sort by name returns the first and second sets of paginated results', async () => {
        ...
        const secondResponse = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'name'}&after=${firstResponse.body[limit - 1]._id}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('sort by releaseDate returns the first and second sets of paginated results', async () => {
        ...
        const secondResponse = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'releaseDate'}&after=${firstResponse.body[limit - 1]._id}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(secondResponse.status).toEqual(HttpStatus.OK);
        expect(secondResponse.body).toHaveLength(2);
      ...
      it('sort by rating returns the first and second sets of paginated results', async () => {
        ...
        const secondResponse = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${rating}&after=${firstResponse.body[limit - 1]._id}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(secondResponse.status).toEqual(HttpStatus.OK);
        expect(secondResponse.body).toHaveLength(2);
    ...
    describe('Validation', () => {
      it('limit should not be empty', async () => {
        const response = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?&sortBy=${'name'}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('sortBy must be one of the following values: name, releaseDate, rating', async () => {
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'tests'}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('responds with error message when an invalid startsWith supplied', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/movies/recently/added?limit=${limit}&sortBy=${'name'}&startsWith=${startsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual({
          statusCode: 400,
