...
export const isNewsPartnerPage = (pathname: string) => new RegExp(`/app/news/partner/${MONGODB_ID}`).test(pathname); // e.g, pathname = '/app/news/partner/6036639a657a566248b973f7'
export const isMovieDetailsPageSubRoutes = (pathname: string) => new RegExp(`/app/movies/${MONGODB_ID}/.+`).test(pathname);
export const isEventDetailsPage = (pathname: string) => new RegExp(`/app/events/${MONGODB_ID}$`).test(pathname); // e.g., pathname = 'http://localhost:3000/app/events/5
export const isConversationPage = (pathname: string) => new RegExp(`/app/messages/conversation/${MONGODB_ID}$`).test(pathname); // e.g., pathname = 'http://localhost:3000/app/events/5
