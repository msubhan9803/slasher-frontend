...
describe('Add Movie User Status Favorite (e2e)', () => {
  ...
  describe('POST /movies/:movieId/lists/favorite', () => {
    it('successfully creates a add movie user status favorite', async () => {
      const response = await request(app.getHttpServer())
        .post(`/movies/${movie.id}/lists/favorite`)
    ...
    it('returns the expected response when the movie id is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .post(`/movies/${movieId}/lists/favorite`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
