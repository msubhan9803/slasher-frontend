...
import { MovieUserStatusBuy } from '../../../../../src/schemas/movieUserStatus/movieUserStatus.enums';
import { rewindAllFactories } from '../../../../helpers/factory-helpers.ts';

describe('Delete Movie User Status Buy (e2e)', () => {
  ...
  describe('Delete /movies/:movieId/lists/buy', () => {
    it('successfully delete a add movie user status buy', async () => {
      const response = await request(app.getHttpServer())
        .delete(`/movies/${movie.id}/lists/buy`)
    ...
    it('returns the expected response when the movie id is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .delete(`/movies/${movieId}/lists/buy`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
