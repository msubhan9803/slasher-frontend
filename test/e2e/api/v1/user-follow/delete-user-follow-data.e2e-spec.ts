...
describe('User-Follow / delete-user-follow-data (e2e)', () => {
    ...
    describe('DELETE /api/v1/user-follow/:followUserId', () => {
        beforeEach(async () => {
        ...
        it('requires authentication', async () => {
            await request(app.getHttpServer()).delete('/api/v1/user-follow/:followUserId').expect(HttpStatus.UNAUTHORIZED);
        });

        describe('Successfully deletes', () => {
            it('can successfully delete the follow user', async () => {
                const sendBody = {
                ...
                const response = await request(app.getHttpServer())
                    .delete(`/api/v1/user-follow/${user1._id}`)
                    .auth(activeUserAuthToken, { type: 'bearer' })
                    .send();
            ...
            it('gives the expected response when data is not existing', async () => {
                const response = await request(app.getHttpServer())
                    .delete(`/api/v1/user-follow/${user2._id}`)
                    .auth(activeUserAuthToken, { type: 'bearer' })
                    .send();
