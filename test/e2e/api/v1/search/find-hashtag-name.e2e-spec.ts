...
describe('Find Hashtag Name (e2e)', () => {
  ...
  describe('GET /api/v1/search/hashtags', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get('/api/v1/search/hashtags').expect(HttpStatus.UNAUTHORIZED);
    });

    describe('Get all hashtag name', () => {
      it('returns suggestions when there are results that match the query (and even includes the '
        + "requesting hashtags's name when the query matches)", async () => {
          ...
          const response = await request(app.getHttpServer())
            .get(`/api/v1/search/hashtags?query=${query}&limit=${limit}`)
            .auth(activeUserAuthToken, { type: 'bearer' })
            .send();
          expect(response.body).toEqual([
            { _id: expect.stringMatching(SIMPLE_MONGODB_ID_REGEX), name: 'good', totalPost: 0 },
    ...
    describe('Validation', () => {
      it('limit should not be empty', async () => {
        const query = 'yy';
        const response = await request(app.getHttpServer())
          .get(`/api/v1/search/hashtags?query=${query}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('limit should be a number', async () => {
        const limit = 'a';
        const response = await request(app.getHttpServer())
          .get(`/api/v1/search/hashtags?query=yy&limit=${limit}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('when query does not exists than expected suggested user name', async () => {
        const limit = 5;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/search/hashtags?limit=${limit}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
