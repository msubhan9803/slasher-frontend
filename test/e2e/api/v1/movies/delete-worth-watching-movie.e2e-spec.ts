...
describe('Movie / Create/Update `rating` for `MovierUserStatus` (e2e)', () => {
  ...
  describe('DELETE /api/v1/movies/:id/worth-watching', () => {
    ...
    it('requires authentication', async () => {
      ...
      await request(app.getHttpServer()).delete(`/api/v1/movies/${movieId}/worth-watching`).expect(HttpStatus.UNAUTHORIZED);
    });
    ...
    describe('validations', () => {
      it('movie does not exist', async () => {
        ...
        const response = await request(app.getHttpServer())
          .delete(`/api/v1/movies/${nonExistingId}/worth-watching`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send({ rating: 3 });
