...
import getBookReviewsResponse from '../../../../fixtures/feed-post/find-book-reviews.response';
...
describe('Feed-Post / Find Book Reviews (e2e)', () => {
  ...
  describe('GET /api/v1/feed-posts/:bookId/bookreviews', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get(`/api/v1/feed-posts/${book.id}/bookreviews`).expect(HttpStatus.UNAUTHORIZED);
    });

    it('returns the expected feed book reviews response', async () => {
      const limit = 5;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/feed-posts/${book.id}/bookreviews?limit=${limit}`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
      for (let i = 1; i < response.body.length; i += 1) {
        expect(response.body[i].createdAt < response.body[i - 1].createdAt).toBe(true);
      }
      expect(response.body).toEqual(getBookReviewsResponse);
    });
    ...
    describe('when `before` argument is supplied', () => {
      ...
      it('get expected first and second sets of paginated results', async () => {
        ...
        const secondResponse = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/${book.id}/bookreviews?limit=${limit}&before=${firstResponse.body[limit - 1]._id}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
    ...
    describe('Validation', () => {
      it('limit should not be empty', async () => {
        const response = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/${book.id}/bookreviews`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('`before` must match regular expression', async () => {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/${book.id}/bookreviews?limit=${limit}&before=${before}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('when feed post is not found than expected feed post response', async () => {
        const limit = 5;
        const bookId = '634fc8d86a5897b88a2d9753';
        const response = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/${bookId}/bookreviews?limit=${limit}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
