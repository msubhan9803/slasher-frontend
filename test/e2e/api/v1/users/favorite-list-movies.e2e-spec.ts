...
describe('Favorite List Movies (e2e)', () => {
  ...
  describe('Get /api/v1/users/:userId/favorite-list', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get(`/api/v1/users/${activeUser.id}/favorite-list`).expect(HttpStatus.UNAUTHORIZED);
    });
    ...
    describe('User all the favorite movies list', () => {
      ...
      it('when startsWith is not exist and nameContains is exist than expected response', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/favorite-list?limit=${limit}&sortBy=${'name'}&nameContains=${nameContains}&startsWith=${sortNameStartsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual([]);
      });
    ...
    describe('Validation', () => {
      it('userId must be a mongodb id', async () => {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${userId}/favorite-list?limit=10&sortBy=name`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('responds with error message when an invalid startsWith supplied', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/favorite-list?limit=${limit}&sortBy=${'name'}&startsWith=${startsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual({
          statusCode: 400,
