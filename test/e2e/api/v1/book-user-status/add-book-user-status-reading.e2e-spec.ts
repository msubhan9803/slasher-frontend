...
describe('Add Book User Status Reading (e2e)', () => {
    ...
    describe('POST /books/:bookId/lists/readingList', () => {
        let book;
        beforeEach(async () => {
        ...
        it('successfully creates a add book user status readingList', async () => {
            const response = await request(app.getHttpServer())
                .post(`/books/${book.id}/lists/readingList`)
        ...
        it('returns the expected response when the bookId is not found', async () => {
            ...
            const response = await request(app.getHttpServer())
                .post(`/books/${bookId}/lists/readingList`)
                .auth(activeUserAuthToken, { type: 'bearer' })
                .send();
