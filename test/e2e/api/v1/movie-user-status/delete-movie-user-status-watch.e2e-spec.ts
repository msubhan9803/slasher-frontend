...
import { MovieUserStatusWatch } from '../../../../../src/schemas/movieUserStatus/movieUserStatus.enums';
import { rewindAllFactories } from '../../../../helpers/factory-helpers.ts';

describe('Delete Movie User Status Watch (e2e)', () => {
  ...
  describe('DELETE /movies/:movieId/lists/watch', () => {
    it('successfully delete a add movie user status watch', async () => {
      const response = await request(app.getHttpServer())
        .delete(`/movies/${movie.id}/lists/watch`)
    ...
    it('returns the expected response when the movie id is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .delete(`/movies/${movieId}/lists/watch`)
        .auth(activeUserAuthToken, { type: 'bearer' })
        .send();
