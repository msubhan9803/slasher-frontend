...
describe('Feed-Post / All Feed Posts (e2e)', () => {
  ...
  describe('GET /api/v1/feed-posts/all/post', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get('/api/v1/feed-posts/all/post').expect(HttpStatus.UNAUTHORIZED);
    });

    it('returns the expected feed post response', async () => {
      const limit = 5;
      const response = await request(app.getHttpServer())
        .get(`/api/v1/feed-posts/all/post?limit=${limit}`)
    ...
    describe('when `before` argument is supplied', () => {
      ...
      it('get expected first and second sets of paginated results', async () => {
        ...
        const secondResponse = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/all/post?limit=${limit}&before=${firstResponse.body[limit - 1]._id}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
    ...
    describe('All Feed should NOT include hidden posts for activeUser', () => {
    ...
    describe('Validation', () => {
      it('limit should not be empty', async () => {
        const response = await request(app.getHttpServer())
          .get('/api/v1/feed-posts/all/post')
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('`before` must match regular expression', async () => {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/v1/feed-posts/all/post?limit=${limit}&before=${before}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
