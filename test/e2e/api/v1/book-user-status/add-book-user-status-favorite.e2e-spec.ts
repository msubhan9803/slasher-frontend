...
import { BookUserStatusFavorites } from '../../../../../src/schemas/bookUserStatus/bookUserStatus.enums';

describe('Add Book User Status Favorite (e2e)', () => {
    ...
    describe('POST /books/:bookId/lists/favorite', () => {
        let book;
        beforeEach(async () => {
        ...
        it('successfully creates a book user status favorite', async () => {
            const response = await request(app.getHttpServer())
                .post(`/books/${book.id}/lists/favorite`)
        ...
        it('returns the expected response when the bookId is not found', async () => {
            ...
            const response = await request(app.getHttpServer())
                .post(`/books/${bookId}/lists/favorite`)
                .auth(activeUserAuthToken, { type: 'bearer' })
                .send();
