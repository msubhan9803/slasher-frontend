...
export class UsersService {
  ...
  async userNameAvailable(userName: string): Promise<boolean> {
    ...
    if (!user || user.status === ActiveStatus.Deactivated
       || (user.deleted && !user.userBanned && !user.userSuspended)) { return true; } // username is available if user not found
    if (user.status === ActiveStatus.Active || user.status === ActiveStatus.Inactive || user.userBanned
      || !user.deleted || user.userSuspended) { return false; } // username not available if user banned or user not deleted
    return true;
  }
  ...
  async emailAvailable(email: string): Promise<boolean> {
    ...
    if (!user || user.status === ActiveStatus.Deactivated
       || (user.deleted && !user.userBanned && !user.userSuspended)) { return true; } // email is available if user not found
    if (user.status === ActiveStatus.Active || user.status === ActiveStatus.Inactive || user.userBanned
      || !user.deleted || user.userSuspended) { return false; } // email not available if user banned or user not deleted
    return true;
  }
