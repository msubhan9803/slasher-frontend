...
describe('Read List Books (e2e)', () => {
  ...
  describe('Get /api/v1/users/:userId/read-booklist', () => {
    it('requires authentication', async () => {
      await request(app.getHttpServer()).get(`/api/v1/users/${activeUser.id}/read-booklist`).expect(HttpStatus.UNAUTHORIZED);
    });

    describe('User all the read booklist', () => {
      it('get all the user read-booklist', async () => {
        const limit = 5;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&&sortBy=name`)
      ...
      it('returns the expected response when the user is not found', async () => {
        ...
        const limit = 2;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${nonExistentUserId}/read-booklist?limit=${limit}&&sortBy=name`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('when user is block than expected response.', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${user0.id}/read-booklist?limit=${limit}&&sortBy=name`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual({
      ...
      it('when name contains supplied than expected all books response', async () => {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'name'}&nameContains=${nameContains}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('when the startsWith is exist than expected response', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'name'}&startsWith=${sortNameStartsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual([{
          _id: expect.stringMatching(SIMPLE_MONGODB_ID_REGEX),
      ...
      it('when startsWith is not exist and nameContains is exist than expected response', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'name'}&nameContains=${nameContains}&startsWith=${sortNameStartsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual([]);
      });
    ...
    describe('should NOT find read list book when users are not friends', () => {
      ...
      it('should not allow the read list books when users are not friends', async () => {
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${user2.id}/read-booklist?limit=${limit}&&sortBy=name`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
    ...
    describe('Validations', () => {
      ...
      it('returns an error if the limit is higher than allowed', async () => {
        const limit = 61;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'publishDate'}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('sortBy must be one of the following values: name, publishDate, rating', async () => {
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'tests'}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('name be shorter than or equal to 30 characters', async () => {
        ...
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'publishDate'}&nameContains=${nameContains}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
      ...
      it('responds with error message when an invalid startsWith supplied', async () => {
        ...
        const limit = 3;
        const response = await request(app.getHttpServer())
          .get(`/api/v1/users/${activeUser.id}/read-booklist?limit=${limit}&sortBy=${'name'}&startsWith=${startsWith}`)
          .auth(activeUserAuthToken, { type: 'bearer' })
          .send();
        expect(response.body).toEqual({
          statusCode: 400,
